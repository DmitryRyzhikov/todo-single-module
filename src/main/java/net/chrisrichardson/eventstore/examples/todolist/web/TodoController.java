...
public class TodoController extends BaseController {
    ...
    public CompletableFuture<ResourceWithUrl> saveTodo(@RequestBody TodoInfo todo,
        ...
        return todoService
                .save(todo)
                .thenApply(e -> withRequestAttributeContext(
                                request, () -> toResource(e.getAggregate().getTodo(), e.getEntityId())
                        )
                );
    }

    @RequestMapping(value = "/{todo-id}", method = DELETE)
    public CompletableFuture<ResourceWithUrl> deleteOneTodo(@PathVariable("todo-id") String id,
            HttpServletRequest request) {
        return todoService.remove(id).thenApply(e -> withRequestAttributeContext(request,
    ...
    public CompletableFuture<ResourceWithUrl> updateTodo(@PathVariable("todo-id") String id,
            @RequestBody TodoInfo newTodo, HttpServletRequest request) {
        return todoService.update(id, newTodo).thenApply(
                e -> withRequestAttributeContext(request, () -> toResource(e.getAggregate().getTodo(), e.getEntityId()))
        );
    }
    ...
    protected ResourceWithUrl toResource(TodoInfo todo, String id) {
        ...
        result.setId(id);
        result.setUrl(
                ControllerLinkBuilder.linkTo(methodOn(TodoViewController.class).getTodo(id)).withSelfRel().getHref());
        return result;
    }
}
